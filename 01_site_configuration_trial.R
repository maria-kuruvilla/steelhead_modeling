...
queryFlowlines = function(sites_sf = NULL,
  ...
  bbox <- sf::st_bbox(c(xmin = -119.9191, ymin = 46.62418, 
                        xmax = -119.4488, ymax = 46.72698),
...
library(ggmap)
register_google(key = "AIzaSyCYQ6M-Ipwe3ef0MyJuWdBv2_a46r0P9nM")
upstrm_comid = ggmap::geocode(upstrm_loc, output = "latlon") %>%
  st_as_sf(coords = c("lon", "lat"),
           crs = 4326) %>%
  nhdplusTools::discover_nhdplus_id()

bbox2 <- sf::st_bbox(c(xmin = -119.6469, ymin = 47.9959, 
                       xmax = -119.5969, ymax = 48.01889),
                     crs = "+proj=longlat +datum=WGS84 +no_defs")
nhd_upstrm_lst = nhdplusTools::plot_nhdplus(bbox = bbox2,
